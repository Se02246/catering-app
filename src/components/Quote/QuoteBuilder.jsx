...
const QuoteBuilder = () => {
    ...
    const handleAiGenerate = async (promptToUse = aiPrompt) => {
        const finalPrompt = typeof promptToUse === 'string' ? promptToUse : aiPrompt;
        ...
        // Initial fallback thoughts while fetching real ones
        let dynamicThoughts = [
            "Analizzando la tua richiesta...",
            "Consultando il catalogo Muse Catering...",
            "Inizializzando il motore creativo..."
        ];
        
        setAiThought(dynamicThoughts[0]);
        let thoughtIndex = 0;
        
        const thoughtInterval = setInterval(() => {
            thoughtIndex = (thoughtIndex + 1) % dynamicThoughts.length;
            setAiThought(dynamicThoughts[thoughtIndex]);
        }, 3500);

        // Fetch real thoughts from AI in parallel
        api.getAiThoughts(finalPrompt).then(thoughts => {
            if (thoughts && thoughts.length >= 5) {
                dynamicThoughts = thoughts;
                // Reset index to start from the first real thought
                thoughtIndex = 0;
                setAiThought(dynamicThoughts[0]);
            }
        }).catch(err => console.error("Error fetching AI thoughts:", err));

        try {
            const data = await api.generateAiQuote(finalPrompt, 'client');
    ...
    useEffect(() => {
        if (!isLoading && rawProducts.length > 0 && location.state?.initialAiPrompt && !hasAutoGenerated.current) {
            hasAutoGenerated.current = true;
            handleAiGenerate(location.state.initialAiPrompt);
            // Pulisci lo state della location per evitare re-trigger su ricaricamenti
            window.history.replaceState({}, document.title);
        }
    }, [isLoading, rawProducts, location.state]);
    ...
    const products = React.useMemo(() => {
        ...
        return rawProducts
            .filter(p => {
                ...
                const notHiddenFromQuotes = !p.hide_from_quotes;

                if (!isVisible || !isNotExpired || !notHiddenFromQuotes) return false;
    ...
    }, [rawProducts, searchTerm, selectedDietaryFilters]);
    ...
    const handleShareQuote = async () => {
        if (!clientName || !clientName.trim()) {
            alert('Per favore, inserisci il tuo Nome e Cognome prima di inviare il preventivo.');
            return;
        }
                ...
                <div className="card-body" style={{ padding: '2rem' }}>
                    {cart.length === 0 ? (
                        ...
                        <>
                            ...
                            <div style={{ padding: '0 0 1.5rem 0', marginBottom: '0' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.8rem' }}>
                            ...
                            <div style={{ padding: '0 0 1.5rem 0', marginBottom: '1rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.8rem' }}>
                                    ...
                                    <h4 style={{ margin: 0, color: 'var(--color-primary-dark)', fontSize: '1rem' }}>Note o Richieste Speciali (Opzionale)</h4>
                                </div>
                                <textarea
                                    placeholder="Scrivi qui eventuali allergie, richieste particolari o messaggi per noi..."
                                    value={notes}
