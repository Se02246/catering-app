...
router.get('/', async (req, res) => {
    try {
        const result = await pool.query('SELECT *, CASE WHEN hide_at IS NOT NULL AND hide_at <= NOW() THEN false ELSE is_visible END as is_visible FROM products ORDER BY name');
...
router.post('/', async (req, res) => {
    const { name, description, price_per_kg, image_url, pieces_per_kg, min_order_quantity, order_increment, show_servings, servings_per_unit, is_visible, images, is_gluten_free, is_lactose_free, is_sold_by_piece, price_per_piece, hide_at } = req.body;
    try {
