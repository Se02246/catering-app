...
router.post('/', async (req, res) => {
    const { name, description, price_per_kg, image_url, pieces_per_kg, min_order_quantity, order_increment, show_servings, servings_per_unit, is_visible, images, is_gluten_free, is_lactose_free, is_sold_by_piece, price_per_piece } = req.body;
    try {
...
router.put('/:id', async (req, res) => {
    const { id } = req.params;
    const { name, description, price_per_kg, image_url, pieces_per_kg, min_order_quantity, order_increment, show_servings, servings_per_unit, is_visible, allow_multiple, images, is_gluten_free, is_lactose_free, is_sold_by_piece, price_per_piece } = req.body;
    try {
        const imagesArray = images || (image_url ? [image_url] : []);
        const mainImage = imagesArray.length > 0 ? imagesArray[0] : image_url;

        const result = await pool.query(
            'UPDATE products SET name = $1, description = $2, price_per_kg = $3, image_url = $4, pieces_per_kg = $5, min_order_quantity = $6, order_increment = $7, show_servings = $8, servings_per_unit = $9, is_visible = $10, allow_multiple = $11, max_order_quantity = $12, images = $13, is_gluten_free = $14, is_lactose_free = $15, is_sold_by_piece = $16, price_per_piece = $17 WHERE id = $18 RETURNING *',
