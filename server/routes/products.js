...
router.post('/', async (req, res) => {
    const { name, description, price_per_kg, image_url, pieces_per_kg, min_order_quantity, order_increment, show_servings, servings_per_unit, is_visible, hide_at, images, is_gluten_free, is_lactose_free, is_sold_by_piece, price_per_piece, hide_quantity } = req.body;
    try {
...
router.put('/:id', async (req, res) => {
    const { id } = req.params;
    const { name, description, price_per_kg, image_url, pieces_per_kg, min_order_quantity, order_increment, show_servings, servings_per_unit, is_visible, hide_at, allow_multiple, images, is_gluten_free, is_lactose_free, is_sold_by_piece, price_per_piece, hide_quantity } = req.body;
    try {
        const imagesArray = images || (image_url ? [image_url] : []);
        const mainImage = imagesArray.length > 0 ? imagesArray[0] : image_url;

        const result = await pool.query(
            'UPDATE products SET name = $1, description = $2, price_per_kg = $3, image_url = $4, pieces_per_kg = $5, min_order_quantity = $6, order_increment = $7, show_servings = $8, servings_per_unit = $9, is_visible = $10, hide_at = $11, allow_multiple = $12, max_order_quantity = $13, images = $14, is_gluten_free = $15, is_lactose_free = $16, is_sold_by_piece = $17, price_per_piece = $18, hide_quantity = $19 WHERE id = $20 RETURNING *',
            [name, description, price_per_kg, mainImage, pieces_per_kg, min_order_quantity || 1, (order_increment !== undefined && order_increment !== null) ? order_increment : 1, show_servings || false, servings_per_unit, is_visible !== undefined ? is_visible : true, hide_at || null, allow_multiple || false, req.body.max_order_quantity || null, imagesArray, is_gluten_free || false, is_lactose_free || false, is_sold_by_piece || false, price_per_piece || null, hide_quantity || false, id]
        );
