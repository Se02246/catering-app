...
router.post('/', async (req, res) => {
    const { name, description, menu_description, price_per_kg, image_url, pieces_per_kg, min_order_quantity, order_increment, show_servings, servings_per_unit, is_visible, hide_at, images, is_gluten_free, is_lactose_free, is_sold_by_piece, price_per_piece, hide_quantity, hide_unit_price, hide_in_menu } = req.body;
    try {
...
router.put('/:id', async (req, res) => {
    const { id } = req.params;
    const { name, description, menu_description, price_per_kg, image_url, pieces_per_kg, min_order_quantity, order_increment, show_servings, servings_per_unit, is_visible, hide_at, allow_multiple, images, is_gluten_free, is_lactose_free, is_sold_by_piece, price_per_piece, hide_quantity, hide_unit_price, hide_in_menu } = req.body;
    try {
        const imagesArray = images || (image_url ? [image_url] : []);
        const mainImage = imagesArray.length > 0 ? imagesArray[0] : image_url;

        const result = await pool.query(
            'UPDATE products SET name = $1, description = $2, menu_description = $3, price_per_kg = $4, image_url = $5, pieces_per_kg = $6, min_order_quantity = $7, order_increment = $8, show_servings = $9, servings_per_unit = $10, is_visible = $11, hide_at = $12, allow_multiple = $13, max_order_quantity = $14, images = $15, is_gluten_free = $16, is_lactose_free = $17, is_sold_by_piece = $18, price_per_piece = $19, hide_quantity = $20, hide_unit_price = $21, hide_in_menu = $22 WHERE id = $23 RETURNING *',
